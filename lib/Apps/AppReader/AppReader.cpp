...
int jpegDrawCallback(JPEGDRAW *pDraw) {
    ...
    for (int y = 0; y < pDraw->iHeight; y++) {
        ...
        for (int x = 0; x < pDraw->iWidth; x++) {
            ...
            if (finalPixel == 0) {
                g_coverBitmap[byteIndex] |= (1 << bitIndex); // Black
            }
            ...
            auto addError = [&](int dx, int dy, int weight) {
                int tx = dstX + dx;
                int ty = dstY + dy;
                if (tx >= 0 && tx < g_targetWidth && ty >= 0 && ty < g_targetHeight && errorBuffer) {
                    errorBuffer[ty * g_targetWidth + tx] += (error * weight) >> 3;
                }
            };
    ...
    if (pDraw->y + pDraw->iHeight >= g_coverHeight && errorBuffer) {
        free(errorBuffer);
        errorBuffer = nullptr;
    }
    return 1;
...
void AppReader::loadSettings() {
    ...
    if (file) {
        ...
        if (!error) {
            if (doc.containsKey("refreshFrequency")) {
                _refreshEveryNPages = doc["refreshFrequency"];
            }
        }
        file.close();
    }
    Serial.printf("Reader Settings: Refresh every %d pages\n", _refreshEveryNPages);
}
...
void AppReader::start() {
    _state = VIEW_LIBRARY;
    // Don't scan here - do it lazily in draw()
    // scanBooks(); 
    _needsRedraw = true;
...
void AppReader::scanBooks() {
    ...
    while(file){
        ...
        if(fileName.endsWith(".epub")) {
            BookEntry entry;
            entry.path = "/" + fileName; // LittleFS paths should start with /

            // Get title from EPUB metadata
            EpubLoader tempLoader;
            if(tempLoader.open(entry.path.c_str())) {
                entry.title = tempLoader.getTitle();
                tempLoader.close();
            } else {
                // Fallback to filename if can't open EPUB
            ...
            if(EbookFS.exists(thumbPath)) {
                ...
                if(thumbFile) {
                    ...
                    if(thumbSize == 600) {  // Expected size: 60x80 / 8 = 600 bytes
                        ...
                        if(!entry.coverBitmap) {
                            entry.coverBitmap = (uint8_t*)malloc(thumbSize);
                        }
                        if(entry.coverBitmap) {
                            thumbFile.read(entry.coverBitmap, thumbSize);
                            entry.hasCover = true;
                            Serial.printf("Loaded thumbnail for: %s\n", entry.title.c_str());
                        }
                    }
                    thumbFile.close();
                }
            ...
            Serial.print("Found Book: "); Serial.println(entry.title);
        }
...
bool AppReader::loadBookCover(BookEntry& book) {
    ...
    if (!coverData || coverSize == 0) {
        Serial.printf("No cover found for %s\n", book.title.c_str());
        return false;
    }

    Serial.printf("Decoding cover for %s (%d bytes)\n", book.title.c_str(), coverSize);

    // Decode JPEG
    JPEGDEC jpeg;
    if (!jpeg.openRAM(coverData, coverSize, jpegDrawCallback)) {
    ...
    Serial.printf("Cover: %dx%d -> %dx%d\n", g_coverWidth, g_coverHeight, g_targetWidth, g_targetHeight);

    // Allocate bitmap for dithered cover (1 bit per pixel)
    int bitmapSize = (g_targetWidth * g_targetHeight + 7) / 8;
    g_coverBitmap = (uint8_t*)ps_malloc(bitmapSize);
    if (!g_coverBitmap) {
        g_coverBitmap = (uint8_t*)malloc(bitmapSize);
    }
    if (!g_coverBitmap) {
        Serial.println("Failed to allocate cover bitmap");
        jpeg.close();
        free(coverData);
        return false;
    }
    memset(g_coverBitmap, 0, bitmapSize);  // White background
    ...
    book.hasCover = true;
    g_coverBitmap = nullptr;  // Ownership transferred

    Serial.printf("Cover loaded for %s\n", book.title.c_str());
    return true;
}
...
void AppReader::drawCover(Book32Display& display, BookEntry& book, int x, int y, int w, int h, bool inverted) {
    ...
    for (int py = 0; py < book.coverHeight && py < h; py++) {
        for (int px = 0; px < book.coverWidth && px < w; px++) {
            ...
            if (inverted) {
                isBlack = !isBlack;  // Invert for selected item
            }
...
void AppReader::handleInput(InputAction action) {
    ...
    if (_state == VIEW_LIBRARY) {
        if (action == INPUT_NEXT) { // Down
            ...
            _needsRedraw = true;
             ...
             if(!_books.empty()) {
                 ...
                 // Don't call draw() here - let it happen in draw() method
             }
        }
    }
        ...
        if (action == INPUT_NEXT) {
            ...
            closeBook(); // Close back to library
             _state = VIEW_LIBRARY;
             _needsRedraw = true;
        }
    }
    
    // Don't call draw() from input handler - causes stack overflow
    // Drawing happens in the draw() method called from main loop
}
...
void AppReader::loadChapter(int chapterIndex) {
    ...
    if (!_textRenderer) {
        ...
        _textRenderer = new TextRenderer(display.width(), display.height(), 2);
    }

    // Try to find a chapter with actual content (skip empty title/cover pages)
    ...
    while (chapterIndex < _epubLoader->getChapterCount()) {
        ...
        // Paginate the rich content
        if(richContent.size() > 0) {
            _pages = _textRenderer->paginateRich(richContent);
        } else {
            // Fallback to plain text if rich content fails
            String chapterText = _epubLoader->getChapterContent(chapterIndex);
            _pages = _textRenderer->paginate(chapterText);
        }

        Serial.printf("Chapter %d loaded: %d pages\n", chapterIndex, _pages.size());

        // If we have pages, we're done
        if (_pages.size() > 0) {
            _needsRedraw = true;
            return;
        }

        // Empty chapter (likely title page/cover), try next one
        Serial.printf("Chapter %d is empty, trying next...\n", chapterIndex);
        chapterIndex++;
    }

    // If we couldn't find any chapter with content, stay at original
...
void AppReader::nextChapter() {
    if (!_epubLoader) return;
    // loadChapter will automatically skip empty chapters forward
...
void AppReader::prevChapter() {
    ...
    if (_currentChapter > 0) {
        // Try previous chapters, skipping empty ones
...
void AppReader::drawLibrary() {
    ...
    const int CHARS_PER_LINE = 30;  // Actual characters that fit in text area
    ...
    do {
        ...
        for (const auto& book : _books) {
            ...
            if (isSelected) {
                display.fillRect(0, y, display.width(), ITEM_HEIGHT, GxEPD_BLACK);
            }

            // Draw book cover
            int coverX = ITEM_PADDING;
            int coverY = y + 10;
            // Need non-const reference for drawCover
            BookEntry& bookRef = _books[idx];
            drawCover(display, bookRef, coverX, coverY, COVER_WIDTH, COVER_HEIGHT, isSelected);

            // Draw book title with word wrap
            ...
            while (pos < titleLen && lineCount < MAX_LINES) {
                ...
                // If not at end, try to break at a space
                ...
                if (lineCount == MAX_LINES - 1 && pos + lineLen < titleLen) {
                    if (line.length() > 3) {
                ...
                if (line.length() > 0) {
                    display.setCursor(TEXT_X, textY);
                    display.print(line);
                    textY += 12;
                    lineCount++;
                }
            ...
            // Draw separator line
            if (!isSelected) {
                display.drawLine(ITEM_PADDING, y + ITEM_HEIGHT - 2, display.width() - ITEM_PADDING, y + ITEM_HEIGHT - 2, GxEPD_BLACK);
            }
        ...
        // Draw scroll indicator if there are more books
        if (_books.size() > 1) {
            display.setTextSize(2);
            display.setTextColor(GxEPD_BLACK);
...
void AppReader::drawReading() {
    // Redraw flag is handled in draw()
    
    if (_pages.empty()) {
        Serial.println("No pages to display");
        return;
    }
    
    if (_currentPage >= _pages.size()) {
        Serial.println("Invalid page index");
        return;
    }
    ...
    // Refresh Logic
    if (_pageTurnsSinceRefresh >= _refreshEveryNPages) {
        display.setFullWindow();
        _pageTurnsSinceRefresh = 0;
    } else {
    ...
    do {
        display.fillScreen(GxEPD_WHITE);
        display.setTextColor(GxEPD_BLACK);
        display.setTextSize(2); // Should match TextRenderer default or be passed
        
        // Render the current page with rich content support
        String pageText = _pages[_currentPage];
        int totalPages = _pages.size();
        
        // Check if this is rich content (starts with "T:" or "TABLE:")
        if(pageText.startsWith("T:") || pageText.startsWith("TABLE:")) {
            _textRenderer->renderRichPage(display, pageText, _currentPage, totalPages);
        } else {
            // Fallback to plain text rendering
            _textRenderer->renderPage(display, pageText, _currentPage, totalPages);
        }
        
    } while (display.nextPage());
     
    Serial.printf("Rendered page %d/%d (Turns: %d/%d)\n", _currentPage + 1, _pages.size(), _pageTurnsSinceRefresh, _refreshEveryNPages);
}
