...
| 3 | ✅ Filtrar itens não-narrativos da contagem de "capítulos" | Baixo (depende de #2) | Médio | Feito em 2026-09-01 como camada informativa (`<guide>` do OPF → `ChapterNarrativeStore` → `/api/toc`), sem tocar `getChapterCount()`/spine: mudar os índices partiria posições de leitura já guardadas em dispositivos reais. Ver docs/plans/2026-09-01-filtrar-capitulos-nao-narrativos-design.md — bem mais envolvido do que "baixo esforço" sugeria, precisamente pela restrição de estabilidade de índices descoberta ao investigar. |
...
| 7 | ✅ Numeração de `<ol>` / indentação de listas aninhadas | Baixo | Baixo | Feito em 2026-09-01: pilha de `<ol>`/`<ul>` em `parseHtmlToRichContent()`, campo `listNumber` no `RichTextNode`, `TextRenderer` imprime "N. " para itens ordenados com indentação por profundidade. |
